...
# 导入ONNX模型
onnx_session = ort.InferenceSession('./fer.onnx')
...
# 获取图像并预处理
def get_frame(frame: Union[cv2.typing.MatLike, cv2.UMat, str]):
...
# 测试单个图像
def test_single(image: Union[cv2.typing.MatLike, cv2.UMat, str], onnx_dir='./fer.onnx'):
    # import time
    session = ort.InferenceSession(onnx_dir)

    input_name = session.get_inputs()[0].name
    # t1 = time.perf_counter()
    opencv_img = get_frame(image)
...
# 测试整个测试集
def test_dataset(dataset_dir, onnx_dir='./fer.onnx'):
    session = ort.InferenceSession(onnx_dir)
    input_name = session.get_inputs()[0].name
...
if __name__ == '__main__':
    ...
    #     print(true_label)
    test_dataset(r'D:\rafdb\test', './fer.onnx')
