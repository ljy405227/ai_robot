...
def visualize(image, results, box_color=(0, 255, 0), text_color=(0, 0, 255), fps=None, is_mark=None):
    output = image.copy()
    landmark_color = [
        (255, 0, 0),  # right eye
        (0, 0, 255),  # left eye
...
def detect_frame(input='./jpg', model=model, is_mark=True):
...
def detect_img(input='./jpg', model=0, is_mark=True):
    # If input is an image
    image = cv.imread(input)
    h, w, _ = image.shape
    ...
    # print(is_mark)
    image = visualize(image, results, is_mark=is_mark)
...
def detec_video(model, is_mark=True):
    deviceId = 0
    ...
    while cv.waitKey(1) < 0:
        ...
        # Draw results on the input image
        frame = visualize(frame, results, fps=tm.getFPS(), is_mark=True)
...
# detect image
# detect_img(input='./24.jpg', model=model, is_mark=True)

# detect video
# detec_video(model, is_mark=True)
