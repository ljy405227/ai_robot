...
n = 0
img_path = "001_sr3.png"
pre_save = img_path.split(".")[0]
...
for det in face_results:
    ...
    if show_chinese:
        ...
        # frame=cv2AddChineseText(frame,'{}:{}'.format(emotion_mapping[fer_result], str(prob)), (bbox[0]-18, bbox[1]-46),(255, 0, 0), 45)
        frame=cv2AddChineseText(frame,'{}:{}'.format(emotion_mapping[fer_result], str(prob)), (bbox[0]-18, bbox[1]-55),(0, 255, 0), 45)
...
# tm.reset()
if cv2.waitKey(0) == ord('s'): # 按's'键保存
    cv2.imwrite("{}_re.png".format(pre_save),frame)
    print("已保存当前图像")
