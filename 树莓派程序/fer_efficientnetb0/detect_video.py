...
# sequential = dnn.readNetFromONNX(onnx_path)  # onnx version
# # sequential = dnn.readNetFromCaffe(caffe_prototxt_path, caffe_model_path)  # caffe model converted from onnx
# input_size = [320,240]
# witdh = input_size[0]
# height = input_size[1]
...
while True:
    ...
    # 人脸检测计时器
    tm.start()
    face_results = detect_model.detect_frame(frame)
    tm.stop()
    # 计算人脸检测器的帧率
    fps = tm.getFPS()
    # 显示帧率
    ...
    for det in face_results:
        ...
        if face.shape[0] == 0 or face.shape[1] == 0:
            continue
        # 人脸表情识别
        fer_result, prob = detect_weight.detect(face)
        # print(fer_result)
        ...
        if show_chinese:
            ...
            frame = cv2AddChineseText(frame, '{} {}'.format(emotion_mapping[fer_result], str(prob)),
                                      (bbox[0], bbox[1] - 26), (255, 0, 0), 25)

        else:
            cv2.putText(frame, '{} {}'.format(fer_result, str(prob)), (bbox[0] - 5, bbox[1] - 12), 0, tl / 3,
